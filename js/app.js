...
function performUndo(){let last=state.drafted.pop();if(!last)return;let p=state.picks.find(x=>x.overall===last.pick);if(p)p.player=null;state.available.push(last.player);state.available.sort((a,b)=>a.rank-b.rank);if(last.wishlistItem&&!state.wishlist.some(x=>x.name.toLowerCase()===last.wishlistItem.name.toLowerCase())){state.wishlist.push(last.wishlistItem);state.wishlist.sort((a,b)=>(a.rank||999)-(b.rank||999))}save();render()}
function performReset(){localStorage.removeItem('aiaDraftRoomV3');state={drafted:[],picks:JSON.parse(JSON.stringify(DATA.picks)),available:JSON.parse(JSON.stringify(DATA.players)),boardMode:'grid',expandedTeam:null,wishlist:[],wishlistHidden:false};closeConfirm();render()}
function openConfirm(type){
  const isUndo=type==='undo';
  if(isUndo&&!state.drafted.length)return;
  const title=isUndo?'Undo Last Pick?':'Reset Draft?';
  const message=isUndo?'Are you sure you want to undo the last pick?':'Are you sure you want to reset the entire draft?';
  const yesAction=isUndo?'confirmYesUndo()':'confirmYesReset()';
  const yesLabel=isUndo?'Yes':'Yes';
  document.body.insertAdjacentHTML('beforeend',`<div id="confirmOverlay" class="confirm-overlay" role="dialog" aria-modal="true" aria-labelledby="confirmTitle"><div class="confirm-box"><div id="confirmTitle" class="confirm-title">${title}</div><div class="confirm-message">${message}</div><div class="confirm-question">Yes or No?</div><div class="confirm-actions"><button id="confirmNo" class="confirm-no" onclick="closeConfirm()">No</button><button class="confirm-yes ${isUndo?'':'confirm-reset'}" onclick="${yesAction}">${yesLabel}</button></div></div></div>`);
  setTimeout(()=>document.getElementById('confirmNo')?.focus(),0);
}
function closeConfirm(){document.getElementById('confirmOverlay')?.remove()}
function confirmYesUndo(){closeConfirm();performUndo()}
function confirmYesReset(){performReset()}
function undo(){openConfirm('undo')}
function resetDraft(){openConfirm('reset')}
...
document.addEventListener('keydown',e=>{
  if(e.key==='Escape'&&document.getElementById('confirmOverlay')){closeConfirm();return;}
