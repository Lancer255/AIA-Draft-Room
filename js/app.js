...
function draft(i){let cur=cp();if(!cur)return;i=Number(i);let p=state.available[i];if(!p)return;cur.player=p;state.available.splice(i,1);state.drafted.push({pick:cur.overall,round:cur.round,team:cur.team,keeper:cur.keeperRound,player:p});save();render()}function undo(){let last=state.drafted.pop();if(!last)return;let p=state.picks.find(x=>x.overall===last.pick);if(p)p.player=null;state.available.push(last.player);state.available.sort((a,b)=>a.rank-b.rank);save();render()}function resetDraft(){if(!confirm('Reset draft?'))return;localStorage.removeItem('aiaDraftRoomV3');state={drafted:[],picks:JSON.parse(JSON.stringify(DATA.picks)),available:JSON.parse(JSON.stringify(DATA.players)),boardMode:'grid',expandedTeam:null,wishlist:[],wishlistHidden:false};render()}
...
function addWishlistByName(name){
  ...
  let p=DATA.players.find(x=>x.name.toLowerCase()===name.toLowerCase())||
        DATA.players.find(x=>x.name.toLowerCase().includes(name.toLowerCase()));
  let item=p?{name:p.name,pos:p.pos,team:p.team,rank:p.rank}:{name:name,pos:'',team:'',rank:999};
  if(!state.wishlist.some(x=>x.name.toLowerCase()===item.name.toLowerCase())){
