...
function draft(i){let cur=cp();if(!cur)return;i=Number(i);let p=state.available[i];if(!p)return;let wishlistItem=state.wishlist.find(x=>x.name.toLowerCase()===p.name.toLowerCase())||null;state.wishlist=state.wishlist.filter(x=>x.name.toLowerCase()!==p.name.toLowerCase());cur.player=p;state.available.splice(i,1);state.drafted.push({pick:cur.overall,round:cur.round,team:cur.team,keeper:cur.keeperRound,player:p,wishlistItem});save();render()}
function performUndo(){let last=state.drafted.pop();if(!last){showNotice('Nothing to Undo','There are no completed picks to undo.');return}let p=state.picks.find(x=>x.overall===last.pick);if(p)p.player=null;state.available.push(last.player);state.available.sort((a,b)=>a.rank-b.rank);if(last.wishlistItem&&!state.wishlist.some(x=>x.name.toLowerCase()===last.wishlistItem.name.toLowerCase())){state.wishlist.push(last.wishlistItem);state.wishlist.sort((a,b)=>(a.rank||999)-(b.rank||999))}save();render()}
function performReset(){localStorage.removeItem('aiaDraftRoomV3');state={drafted:[],picks:JSON.parse(JSON.stringify(DATA.picks)),available:JSON.parse(JSON.stringify(DATA.players)),boardMode:'grid',expandedTeam:null,wishlist:[],wishlistHidden:false};render()}
function closeConfirm(){let modal=document.getElementById('confirmModal');if(modal)modal.remove()}
function showConfirm(title,message,yesLabel,onYes,danger){closeConfirm();let modal=document.createElement('div');modal.id='confirmModal';modal.className='confirm-overlay';let box=document.createElement('div');box.className='confirm-box';let heading=document.createElement('div');heading.className='confirm-title';heading.textContent=title;let body=document.createElement('div');body.className='confirm-message';body.textContent=message;let actions=document.createElement('div');actions.className='confirm-actions';let noBtn=document.createElement('button');noBtn.textContent='No';noBtn.className='confirm-no';noBtn.addEventListener('click',closeConfirm);let yesBtn=document.createElement('button');yesBtn.textContent=yesLabel||'Yes';yesBtn.className=danger?'danger':'primary';yesBtn.addEventListener('click',()=>{closeConfirm();onYes()});actions.append(noBtn,yesBtn);box.append(heading,body,actions);modal.append(box);modal.addEventListener('click',e=>{if(e.target===modal)closeConfirm()});document.body.append(modal);setTimeout(()=>noBtn.focus(),0)}
function showNotice(title,message){closeConfirm();let modal=document.createElement('div');modal.id='confirmModal';modal.className='confirm-overlay';let box=document.createElement('div');box.className='confirm-box';let heading=document.createElement('div');heading.className='confirm-title';heading.textContent=title;let body=document.createElement('div');body.className='confirm-message';body.textContent=message;let actions=document.createElement('div');actions.className='confirm-actions';let okBtn=document.createElement('button');okBtn.textContent='OK';okBtn.className='primary';okBtn.addEventListener('click',closeConfirm);actions.append(okBtn);box.append(heading,body,actions);modal.append(box);document.body.append(modal);setTimeout(()=>okBtn.focus(),0)}
function requestUndo(){showConfirm('Undo Last Pick?','Are you sure you want to undo the last draft pick?','Yes',performUndo,false)}
function requestReset(){showConfirm('Reset Draft?','Are you sure? This will clear the entire draft board and all draft progress.','Yes',performReset,true)}
...
function render(){let cur=cp(),mj=nextFor('MJIJX');document.getElementById('app').innerHTML=`<div class="app"><div class="status"><div><span class="label">Round</span>${cur?cur.round:'-'}</div><div><span class="label">Pick</span>${cur?cur.overall:'-'}</div><div><span class="label">On the Clock</span>${cur?cur.team:'Draft Complete'}</div><div><span class="label">MJIJX</span>${mj&&cur?(mj.overall-cur.overall)+' picks':'-'}</div><div class="status-intel"><span class="status-intel-title">Draft Radar / Decision Center</span><span class="status-actions"><button onclick="requestUndo()">Undo</button><button class="danger" onclick="requestReset()">Reset</button></span></div></div><div class="tracker">${trackerTeams().map((t,i)=>renderTrackerCard(t,i===0)).join('')}</div><div class="main"><section class="panel"><div class="panel-head">Available Players <span class="small">ADP / Value</span></div><div class="filters"><select id="pos" onchange="renderPlayers()"><option>ALL</option><option>RB</option><option>WR</option><option>QB</option><option>TE</option><option>KR</option><option>DE</option></select><input id="search" oninput="renderPlayers()" placeholder="Search player"></div><div class="scroll" id="players"></div></section><section class="panel"><div class="panel-head">Live Draft Board <span class="small">CBS-style grid</span></div><div class="toolbar"><button onclick="setBoard('grid')" class="${state.boardMode==='grid'?'primary':''}">Grid</button><button onclick="setBoard('list')" class="${state.boardMode==='list'?'primary':''}">Recent</button><button onclick="setBoard('team')" class="${state.boardMode==='team'?'primary':''}">Teams</button></div><div class="scroll" id="board"></div></section><section class="panel intelligence-panel">${renderWishlist()}<div class="war">${renderWar()}</div></section></div></div>`;renderPlayers();renderBoard()}
