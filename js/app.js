...
function renderWar(){let cur=cp();if(!cur)return '<div class="card">Draft complete.</div>';let teamRoster=roster('MJIJX');let starterFilled=needs('MJIJX').reduce((sum,n)=>sum+Math.min(n.have,n.need),0);let benchFilled=Math.max(0,teamRoster.length-starterFilled);let rosterTotal=teamRoster.length;let picksRemaining=Math.max(0,17-rosterTotal);let myNeeds=needs('MJIJX').map(n=>{let status=n.have===0?'need-empty':(n.have>=n.need?'need-filled':'need-partial');return `<div class="need-box ${status}"><b>${n.pos}</b><br>${n.have}/${n.need}</div>`}).join('');let benchStatus=benchFilled===0?'need-empty':(benchFilled>=8?'need-filled':'need-partial');return `<div class="card"><div class="card-title">MJIJX Needs</div><div class="need-grid">${myNeeds}<div class="need-box bench-box ${benchStatus}"><b>BENCH</b><span>${benchFilled}/8</span></div></div><div class="roster-summary"><div class="roster-stat"><span>Roster</span><b>${rosterTotal}/17</b></div><div class="roster-stat"><span>Picks Remaining</span><b>${picksRemaining}</b></div></div></div>${cur.team==='MJIJX'?renderDecisionMode():renderRadarMode()}`}
function render(){let cur=cp(),mj=nextFor('MJIJX');document.getElementById('app').innerHTML=`<div class="app"><div class="status"><div><span class="label">Round</span>${cur?cur.round:'-'}</div><div><span class="label">Pick</span>${cur?cur.overall:'-'}</div><div><span class="label">On the Clock</span>${cur?cur.team:'Draft Complete'}</div><div><span class="label">MJIJX</span>${mj&&cur?(mj.overall-cur.overall)+' picks':'-'}</div><div><button onclick="undo()">Undo</button> <button class="danger" onclick="resetDraft()">Reset</button></div></div><div class="tracker">${trackerTeams().map((t,i)=>renderTrackerCard(t,i===0)).join('')}</div><div class="main"><section class="panel"><div class="panel-head">Available Players <span class="small">ADP / Value</span></div><div class="filters"><select id="pos" onchange="renderPlayers()"><option>ALL</option><option>RB</option><option>WR</option><option>QB</option><option>TE</option><option>KR</option><option>DE</option></select><input id="search" oninput="renderPlayers()" placeholder="Search player"></div><div class="scroll" id="players"></div></section><section class="panel"><div class="panel-head">Live Draft Board <span class="small">CBS-style grid</span></div><div class="toolbar"><button onclick="setBoard('grid')" class="${state.boardMode==='grid'?'primary':''}">Grid</button><button onclick="setBoard('list')" class="${state.boardMode==='list'?'primary':''}">Recent</button><button onclick="setBoard('team')" class="${state.boardMode==='team'?'primary':''}">Teams</button></div><div class="scroll" id="board"></div></section><section class="panel right-panel"><div class="panel-head">Draft Radar / Decision Center</div><div class="right-panel-scroll">${renderWishlist()}<div class="war">${renderWar()}</div></div></section></div></div>`;renderPlayers();renderBoard()}
